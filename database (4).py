...
def reset_password(username, email, new_password):
    """
    Reset a user's password after verifying the username/email pair.
    Returns: (success: bool, message: str)
    """
    ...
    try:
        ...
        if result is None:
            conn.close()
            return False, "No account found with that username and email"

        password_hash = bcrypt.hashpw(new_password.encode('utf-8'), bcrypt.gensalt()).decode('utf-8')

        cursor.execute(
            "UPDATE users SET password_hash = %s WHERE username = %s AND email = %s",
